...
import { HttpErrorResponse, HttpClient } from '@angular/common/http'; // Importação do HttpClient
import { CommonModule } from '@angular/common';
import { IbgeService } from '../ibge.service';
import { NationalityService } from '../services/services2/nationality.service'; // Importe o serviço de nacionalidades
...
export class EditListComponent implements OnInit {
  ...
  beds: { id: number, bed_number: string, is_available: boolean }[] = [];
  filteredBeds: { id: number, bed_number: string, is_available: boolean }[] = [];
  nacionalidades: string[] = []; // Adicione um array para armazenar as nacionalidades
  isLoading: boolean = false;
  isSaving: boolean = false;
  showModal: boolean = false;
  modalMessage: string = '';
  currentStep: number = 1;
  errorMessage: string = ''; // Adicionado para lidar com erros de autenticação

  // Lista de doenças crônicas para o select
  ...
  constructor(
    private http: HttpClient, // HttpClient injetado
    private appointmentService: AppointmentService,
    private ibgeService: IbgeService,
    private nationalityService: NationalityService, // Injete o serviço de nacionalidades
  ...
  ngOnInit(): void {
    ...
    this.loadEstados();
    this.loadRooms();
    this.loadNationalities(); // Carrega as nacionalidades ao iniciar
  }

  // Função para carregar as nacionalidades
    ...
    if (token) {
      this.http.get<Appointment>(`http://127.0.0.1:8000/api/appointments/${id}`, {
        ...
        (response: Appointment) => {
          ...
          // Verifique se additionalInfo está presente
  ...
  onSubmit(): void {
    ...
    this.appointmentService.updateAppointment(this.appointment.id, combinedData).subscribe(
      ...
      (error: HttpErrorResponse) => {
        ...
        this.isSaving = false;
        this.modalMessage = 'Erro ao atualizar o agendamento. Tente novamente mais tarde.';
        this.showModal = true;
      }
    );
  }
